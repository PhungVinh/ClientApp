...
import { selectMenu, selectTab } from 'src/app/core/auth/auth.selectors';
import { ActionAddOrActiveTab } from 'src/app/core/auth/auth.actions';
import { LocalStorageService } from 'src/app/core/local-storage/local-storage.service';
import { AUTH_KEY, AUTH_TOKEN } from 'src/app/core/auth/auth.constants';
import { Router } from '@angular/router';
import { ActionAuthLogout } from "../../core/auth/auth.actions";
import { OganizationDeleteComponent } from "../../theme/admin/single-page-on/tabs/components/organization/oganization-delete.component";
import { NgbModal } from "@ng-bootstrap/ng-bootstrap";
import { ChangePasswordUerComponent } from "../../theme/auth/change-password-uer/change-password-uer.component";
import {selectAccount} from '../../core/auth/auth.selectors';
import {environment} from '@env/environment';
import {selectProfileLinkImage} from '../../theme/admin/single-page-on/tabs/selectors/profie.selectors';
...
export class AdminComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    console.log('chay vao admin');
    this.setBackgroundPattern('theme1');
    this.getUser();
      const authToken = this.localStorageService.getItem(AUTH_TOKEN);
  ...
  getUser() {
      ...
      // this.store.pipe(select(selectProfileLinkImage)).subscribe(data => {
      //   const datas = Object.assign({}, data);
      //     this.avatar = this.resourceAPI + datas.data;
      //     console.log('avarta 2', this.avatar);
      // });
  }
  ...
  onOpenTab(asideChildren) {
    this.store.dispatch(new ActionAddOrActiveTab({ tab: Object.assign({}, asideChildren) }));
  }

  onOpenModal(asideChildren) {
    if (asideChildren.code === 'PROFILE_CHANGEPASSWORD') {
      const modalRef = this.modalService.open(ChangePasswordUerComponent as Component, { backdrop: 'static' });
    }
  }
  openInfoOrg() {
    const tab = {
      'badge': null,
      'children': [],
      'code': 'INFOR_ORGANIZATION',
      'icon': null,
      'mainState': null,
      'menuId': 20,
      'name': 'Thông tin đơn vị',
      'parentCode': 'INFOR_ORGANIZATION',
      'shortLabel': 'DV',
      'state': null,
      'target': null,
      'type': 'tab',
    };
    this.store.dispatch(new ActionAddOrActiveTab({ tab: Object.assign({}, tab) }));
  }
  Logout(evt) {
